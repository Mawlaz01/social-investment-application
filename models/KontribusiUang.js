...
class KontribusiUang {
    static async getAllByAcaraId(acaraId) {
        return new Promise((resolve, reject) => {
            connection.query(`
                SELECT ku.*, k.tanggal_sumbangan, k.tanggal_edit_sumbangan, k.status_validasi, u.nik, u.nama AS nama_penyumbang, u.no_wa  -- tambahkan u.no_wa di sini
    ...
    static async getUnvalidatedByAcaraId(acaraId) {
        return new Promise((resolve, reject) => {
            connection.query(`
                SELECT ku.*, k.tanggal_sumbangan, k.tanggal_edit_sumbangan, k.status_validasi, u.nik, u.nama AS nama_penyumbang
                FROM Kontribusi_Uang ku
                JOIN Kontribusi k ON ku.id_kontribusi = k.id_kontribusi
                JOIN User u ON k.id_penyumbang = u.id_user
    ...
    static async getAllByUserIdAndAcaraId(userId, acaraId) {
        return new Promise((resolve, reject) => {
            const query = `
                SELECT Kontribusi_Uang.*, User.NIK, User.nama AS nama_penyumbang, User.no_wa, 
