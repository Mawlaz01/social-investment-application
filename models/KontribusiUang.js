...
class KontribusiUang {
    static async getAllByAcaraId(acaraId) {
        return new Promise((resolve, reject) => {
            connection.query(`SELECT ku.*, k.tanggal_sumbangan, k.tanggal_edit_sumbangan, k.status_validasi, u.nik, u.nama AS nama_penyumbang, u.no_wa  -- tambahkan u.no_wa di sini FROM Kontribusi_Uang ku JOIN Kontribusi k ON ku.id_kontribusi = k.id_kontribusi JOIN User u ON k.id_penyumbang = u.id_user WHERE k.id_acara = ?`, [acaraId], (err, rows) => {
                if (err) reject(err);
                else resolve(rows);
            });
        });
