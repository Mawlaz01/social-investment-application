...
class KontribusiBarang {
    static async getAllByAcaraId(acaraId) {
        return new Promise((resolve, reject) => {
            connection.query(`SELECT kb.*, k.tanggal_sumbangan, k.tanggal_edit_sumbangan, k.status_validasi, u.nik, u.nama AS nama_penyumbang, u.no_wa 
    ...
    static async getUnvalidatedByAcaraId(acaraId) {
        return new Promise((resolve, reject) => {
            connection.query(`SELECT kb.*, k.tanggal_sumbangan, k.tanggal_edit_sumbangan, k.status_validasi, u.nik, u.nama AS nama_penyumbang 
                              FROM Kontribusi_Barang kb 
                              JOIN Kontribusi k ON kb.id_kontribusi = k.id_kontribusi 
                              JOIN User u ON k.id_penyumbang = u.id_user 
    ...
    static async getAllByUserIdAndAcaraId(userId, acaraId) {
        return new Promise((resolve, reject) => {
            const query = `SELECT Kontribusi_Barang.*, User.NIK, User.nama AS nama_penyumbang, User.no_wa, 
    ...
    static async getTotalBarangByUserId(userId) {
        return new Promise((resolve, reject) => {
            const query = `SELECT COUNT(*) AS totalBarang FROM Kontribusi_Barang kb 
                           JOIN Kontribusi k ON kb.id_kontribusi = k.id_kontribusi 
                           WHERE k.id_penyumbang = ? AND k.status_validasi = 'valid'`;
    ...
    static async getTotalBarangByCreatorId(userId) {
        return new Promise((resolve, reject) => {
            const query = `SELECT COUNT(*) AS totalBarang FROM Kontribusi_Barang kb 
                           JOIN Kontribusi k ON kb.id_kontribusi = k.id_kontribusi 
                           JOIN Acara a ON k.id_acara = a.id_acara 
                           WHERE a.id_pembuat_acara = ? AND k.status_validasi = 'valid'`;
            connection.query(query, [userId], (err, rows) => {
                if (err) reject(err);
                else resolve(rows[0].totalBarang);
            });
